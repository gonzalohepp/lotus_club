    ...
    return L.divIcon({
        ...
        html: `
            <div class="relative flex items-center justify-center">
                ${isSelected ? '<div class="absolute w-14 h-14 bg-white/40 rounded-full animate-ping"></div>' : ''}
    ...
    return L.divIcon({
        ...
        html: `
            <div class="relative flex items-center justify-center w-6 h-6">
                <div class="absolute w-full h-full bg-white/50 rounded-full animate-ping"></div>
    ...
    return L.divIcon({
        ...
        html: `
            <div style="position: relative; display: flex; align-items: center; justify-content: center; width: 48px; height: 48px;">
                <div style="position: absolute; width: 40px; height: 40px; background: rgba(255, 255, 255, 0.3); border-radius: 50%; animation: pulse 2s infinite;"></div>
                <div style="position: relative; width: 36px; height: 36px; background: #000000; border: 3px solid white; border-radius: 50%; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3); display: flex; align-items: center; justify-content: center; color: white; font-weight: 900; font-size: 14px; letter-spacing: -0.5px;">
                   +${count}
                </div>
            </div>
            <style>
        ...
        <div className="flex flex-col lg:flex-row h-[600px] md:h-[700px] w-full rounded-[2rem] md:rounded-[32px] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl relative bg-white dark:bg-slate-950">
            ...
            <div className="lg:hidden flex p-2 gap-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'list' ? 'bg-black text-white shadow-lg' : 'text-slate-500'}`}
                >
                ...
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'map' ? 'bg-black text-white shadow-lg' : 'text-slate-500'}`}
                >
                    <div className="flex items-center gap-2">
            ...
            <div className={`w-full lg:w-80 bg-white dark:bg-slate-900 flex flex-col border-r border-slate-200 dark:border-slate-800 z-20 ${viewMode === 'map' ? 'hidden lg:flex' : 'flex'}`}>
                <div className="p-6 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="bg-slate-200 dark:bg-slate-700 text-black dark:text-white text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider">
                            Argentina
                        </span>
                    </div>
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full h-10 pl-10 pr-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-sm font-medium focus:ring-2 ring-black/20 outline-none transition-all"
                            value={search}
                    ...
                    <button
                        ...
                        disabled={isLocating}
                        className="w-full h-10 bg-slate-100 dark:bg-slate-800 hover:bg-black dark:hover:bg-white text-black dark:text-white hover:text-white dark:hover:text-black rounded-xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700 transition-all active:scale-95"
                    >
                        {isLocating ? (
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
                    {filtered.map(academy => (
                        <motion.div
                            ...
                            `}
                        >
                            <h4 className={`font-bold text-sm mb-1 ${selected?.id === academy.id ? 'text-white' : 'text-slate-800 dark:text-slate-200 group-hover:text-black'}`}>
                                {academy.name}
                            </h4>
                            <div className={`flex items-start gap-1.5 text-[10px] font-medium leading-tight ${selected?.id === academy.id ? 'text-white/70' : 'text-slate-500'}`}>
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto scrollbar-hide">
                                ...
                                <div className="h-48 relative bg-slate-100 dark:bg-slate-800">
                                    ...
                                    <div className="absolute bottom-6 left-6 right-6">
                                        <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.3em] mb-1">Sede Oficial</p>
                            ...
                            <div className="p-6 border-t border-slate-100 dark:border-slate-800 space-y-3 bg-slate-50/50 dark:bg-slate-900/50">
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="flex items-center justify-center gap-3 w-full h-12 bg-black hover:bg-slate-800 text-white font-black rounded-xl text-xs uppercase tracking-[0.2em] shadow-xl shadow-black/20 transition-all hover:-translate-y-0.5"
                                >
