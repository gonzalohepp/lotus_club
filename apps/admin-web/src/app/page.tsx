...
import { ArrowUp, Instagram, MapPin, Menu, MessageCircle, X } from "lucide-react"
...
/**
 * Landing pública de Lotus Club.
 *
 * El mapa de sedes (`AcademiesMapSection`) es lo único que sale de la base:
 * lee `dojos` filtrando por `NEXT_PUBLIC_LANDING_ORG`, así que las sucursales
 * se actualizan solas desde /superadmin. El resto del contenido es editorial y
 * está en el JSX — si en algún momento cada marca necesita su propia landing,
 * estos textos tienen que salir a la base.
 */
...
export default function HomeLandingPage() {
  ...
  /**
   * Analítica de la landing. Se conserva de la versión anterior: es la fuente
   * de `landing_events`, que alimenta las métricas del panel.
   */
  const trackEvent = useCallback(async (eventType: string, metadata?: Record<string, unknown>) => {
    try {
      ...
      try {
        ...
        /* la IP es opcional: si el servicio no responde, se registra igual */
      }
      ...
      // La analítica nunca debe romper la landing.
    }
  }, [])
          ...
          >
            ...
            <div className="space-y-8 text-white/70 text-lg md:text-xl leading-relaxed font-medium">
              ...
              <div className="p-8 bg-white/5 border border-white/10 rounded-3xl italic">
                &ldquo;El principal objetivo del jiu-jitsu brasileño es someter al rival mediante una luxación o estrangulación sin necesidad de usar golpes, de ahí que se le denomine a veces «arte suave».&rdquo;
              </div>
              <p>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="space-y-12 text-white/60 text-lg leading-relaxed">
            ...
            <div className="space-y-6">
              ...
              <p>
                Mitsuyo Maeda, conocido como el &ldquo;Conde Koma&rdquo;, fue uno de los grandes judokas del Kodokan que recorrieron el mundo aceptando retos. En 1914 llegó a Belem do Pará, Brasil, donde Gastão Gracie le ayudó en sus exhibiciones. Maeda aceptó entrenar a los hijos de Gastão como agradecimiento.
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <p>
                En 1925 se abrió la primera academia de &ldquo;Gracie Jiu-jitsu&rdquo; en Río de Janeiro. Carlos Gracie y sus hermanos perfeccionaron las técnicas y desafiaban a cualquiera a un combate sin reglas para demostrar la eficacia del sistema. Hélio Gracie, dadas sus condiciones físicas, adaptó y perfeccionó aún más las técnicas para maximizar el uso de palancas y minimizar el esfuerzo físico.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-4 cursor-pointer group" onClick={() => scrollToSection("inicio")}>
              ...
              <img
                src="/logo.png"
                alt="Lotus Club"
                className="h-20 w-auto brightness-0 invert opacity-80 transition-all duration-500 group-hover:opacity-100 group-hover:scale-105"
              />
            </div>
            <div className="flex flex-col md:flex-row items-center gap-6 text-white/40">
              <a
                href="https://www.instagram.com/lotusclub_ar"
                target="_blank"
