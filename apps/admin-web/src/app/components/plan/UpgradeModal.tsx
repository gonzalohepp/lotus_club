...
function FeatureCell({ included = false, override }: { included?: boolean, override?: string }) {
    if (override) {
        return <span className="text-xs font-bold text-carbon-600 dark:text-carbon-300 whitespace-nowrap">{override}</span>
    }
    return included
        ? <Check className="w-4 h-4 text-kuro-500 shrink-0" strokeWidth={3} />
        : <Minus className="w-4 h-4 text-carbon-300 dark:text-carbon-700 shrink-0" strokeWidth={3} />
}
            ...
            {open && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-3xl overflow-hidden rounded-[32px] bg-white dark:bg-carbon-900 shadow-2xl max-h-[90vh] flex flex-col"
                    >
                        {/* Header */}
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar px-8 py-6">
                            <div className="grid md:grid-cols-2 gap-6">
                                {/* Basic column */}
                                <div className="rounded-3xl border border-carbon-200 dark:border-carbon-800 p-6">
                                    <div className="flex items-center justify-between mb-1 gap-2">
                                        <h3 className="text-lg font-black text-carbon-900 dark:text-white uppercase tracking-tight">Basic</h3>
                                        <span className="text-[10px] font-black uppercase tracking-widest text-carbon-400 bg-carbon-100 dark:bg-white/5 px-3 py-1 rounded-full whitespace-nowrap">
                                            Tu plan actual
                                        </span>
                                    </div>
                                    <p className="text-xs text-carbon-400 font-bold uppercase tracking-widest mb-6">Lo esencial para arrancar</p>
                                    <ul className="space-y-3">
                                        {FEATURE_ORDER.map(key => (
                                            <li key={key} className="flex items-center justify-between gap-3 text-sm">
                                                <span className="text-carbon-600 dark:text-carbon-300 font-medium">{FEATURE_LABELS[key]}</span>
                                                {key === 'dojos'
                                ...
                                {/* Pro column */}
                                <div className="relative rounded-3xl border-2 border-kuro-500/40 p-6 bg-gradient-to-b from-kuro-50/60 to-transparent dark:from-kuro-500/10 dark:to-transparent shadow-lg shadow-kuro-500/10">
                                    <div className="absolute -top-3 left-6 px-3 py-1 rounded-full bg-kuro-600 text-white text-[10px] font-black uppercase tracking-widest shadow-md">
                                        Recomendado
                                    </div>
                                    <div className="flex items-center justify-between mb-1 pt-1">
                                        <h3 className="text-lg font-black text-kuro-700 dark:text-kuro-400 uppercase tracking-tight">Pro</h3>
                                    </div>
                                    <p className="text-xs text-carbon-400 font-bold uppercase tracking-widest mb-6">Todo el poder del panel</p>
                                    <ul className="space-y-3">
                                        {FEATURE_ORDER.map(key => (
                                            <li key={key} className="flex items-center justify-between gap-3 text-sm">
                                                <span className="text-carbon-700 dark:text-carbon-200 font-semibold">{FEATURE_LABELS[key]}</span>
                                                {key === 'dojos'
                        ...
                        {/* Footer CTA */}
                        <div className="shrink-0 px-8 py-6 border-t border-carbon-100 dark:border-carbon-800 flex flex-col md:flex-row items-center justify-between gap-4 bg-carbon-50/50 dark:bg-white/[0.02]">
                            <p className="text-xs font-bold text-carbon-400 uppercase tracking-widest text-center md:text-left">
                                Te contamos precio y tiempos de activación a medida.
                            </p>
                            <a
                                href={UPGRADE_CONTACT_URL}
                                className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-3.5 rounded-2xl bg-kuro-600 hover:bg-kuro-700 text-white font-black uppercase tracking-widest text-xs shadow-xl shadow-kuro-500/25 transition-all hover:scale-105 active:scale-95"
                            >
